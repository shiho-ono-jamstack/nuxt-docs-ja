...
- このアップストリームの Vue バグにより、ビューの遷移がネストされたページ/レイアウト/非同期コンポーネントで期待どおりに動作しない可能性があります: <https://github.com/vuejs/core/issues/5513>。 このパターンを利用する場合は、この実験的な機能の採用を遅らせるか、自分で実装する必要があるかもしれません。 フィードバックは大歓迎です。
